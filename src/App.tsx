...
// --- Skip Card Component ---
const SkipCard = ({ skip, selectedSkip, onSelect }: { skip: Skip, selectedSkip: Skip | null, onSelect: (skip: Skip) => void }) => {
      ...
      <div className="relative">
        ...
        <div className="absolute bottom-3 left-2 z-10 flex flex-col gap-2">
          ...
          {!skip.allows_heavy_waste && (
            <div className="bg-black/90 backdrop-blur-sm px-3 py-1.5 rounded-lg flex items-center gap-2">
              <AlertTriangleIcon />
              <span className="text-xs font-medium text-yellow-500">No Heavy Waste</span>
            </div>
          )}
        </div>
      </div>

      <h3 className="text-xl font-bold mb-2 text-white">{skip.size} Yard Skip</h3>
      <p className="text-sm text-gray-400 mb-6">{skip.hire_period_days} day hire period</p>

      <div className="flex justify-between items-center mb-4">
        <div>
          {skip.price_before_vat !== null ? (
            <>
              <span className="text-2xl font-bold text-blue-600">£{skip.price_before_vat.toFixed(2)}</span>
        ...
        <div className="flex items-center gap-6 mb-4 sm:mb-0">
          ...
          <div>
            {selectedSkip.price_before_vat !== null ? (
              <>
                <span className="text-2xl font-bold text-blue-600">£{selectedSkip.price_before_vat.toFixed(2)}</span>
                <span className="text-sm text-gray-400 ml-2">per week</span>
              </>
            ) : (
              <span className="text-2xl font-bold text-blue-600">POA</span>
